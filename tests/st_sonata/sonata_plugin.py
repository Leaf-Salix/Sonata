...
def _extract_program_from_module(module: Any) -> Any | None:
    """Find the first @pl.program decorated class in a module."""
    import pypto.language as pl

    for name in dir(module):
        obj = getattr(module, name)
        if isinstance(obj, type) and hasattr(obj, "_pypto_program"):
            return obj
    return None
...
def pytest_runtest_setup(item: pytest.Item) -> None:
    ...
    test_name = item.name
    verbose = not item.config.option.quiet

    if verbose:
        sys.stdout.write(f"\n[SONATA] {test_name}: extracting certified dump...\n")
        sys.stdout.flush()
    ...
    if analysis is None:
        if verbose:
            sys.stdout.write(f"[SONATA] {test_name}: Sonata not available, skipping analysis\n")
        return

    if "error" in analysis:
        if verbose:
            sys.stdout.write(f"[SONATA] {test_name}: analysis error: {analysis['error']}\n")
        return
    ...
    if verbose:
        ...
        if analysis.get("has_plan"):
            sys.stdout.write(f"[SONATA] {test_name}: HostBuildGraphPlan generated\n")
        sys.stdout.flush()
