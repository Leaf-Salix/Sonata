...
extern "C" int aicpu_execute(void* prebuilt_arena, uint64_t /*arena_size*/,
    ...
    //
    // The host-side (bind_callable_to_runtime_impl) has set up the GM heap
    // and SM but NOT the arena layout — we do that here (matching the
    // upstream TMARB pattern where aicpu_execute handles runtime init).
